...
function updateHtmlTodo() {
  ...
  let container = document.getElementById("small_card_todo");
  container.innerHTML = "";
  if (todo.length < 1) {
    container.innerHTML = renderNoTasksToDo();
  } else {
    for (let index = 0; index < todo.length; index++) {
      const elementToDo = todo[index];
      container.innerHTML += renderTaskCardToDo(elementToDo);
...
function addPrioImg(task) {
  ...
  const prio_small = document.getElementById(`prio_small_${task.id}`);
  if (prioimg) {
